...
import EditUsuario from "examples/Cards/Forms/Usuarios/editUser";
import axios from "axios";
import Swal from "sweetalert2";
import { CircularProgress } from "@mui/material";

function UpdateUser({ close, show, fetch, id_usuario }) {
  const id = id_usuario;
  ...
  const actualizarFormulario = async () => {
    try {
      const response = await axios.get(`${API_Host}/api/selectUser/${id}`);
      if (response.status === 201) {
        setFormDataUsuarios((prev) => ({
          ...prev,
          nuser: response.data.nuser,
  ...
  const updateUser = async () => {
    ...
    if (personaExist) {
      ...
      try {
        ...
        const responseUsuario = await axios.post(`${API_Host}/api/updateUser/${formDataUsuarios}`, {
          timeout: 5000,
        });
    ...
    <Modal size="lg" show={show} onHide={close} backdrop="static" keyboard={false} centered>
      ...
      <Modal.Footer>
        {cargando ? (
          ...
          <>
            <Button variant="primary" onClick={updateUser} disabled={!isFormValid || cargando}>
              Editar Usuario
            </Button>
